...
char bit_criteria_co2 = get_bit_criteria2(binaries.size(), binaries.size()-zero_counter);
...
while(o_generator.size()>1){
    ...
    j++;
    bit_criteria_o=get_bit_criteria(accepted_binaries.size(), zero_counter);
...
while(co2_rubber.size()>1){
    ...
    j++;
    bit_criteria_co2=get_bit_criteria2(accepted_binaries.size(), accepted_binaries.size()-zero_counter);
